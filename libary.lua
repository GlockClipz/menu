		-- [[ // Error Handling // ]]
			...
			local library = {
				Renders = {},
				Connections = {},
				Folder = "juju.lol", -- Change if wanted
			...
			function library:CreateWindow(Properties)
				...
				local Window = {
					Pages = {},
					Accent = Color3.fromRGB(156,187,78), -- Color3.fromRGB(136, 180, 57) -- Change if wanted
					Enabled = true,
					...
					do -- // Connections
						utility:CreateConnection(uis.InputBegan, function(Input)
							if Input.KeyCode and Input.KeyCode == Window.Key then
								if game:GetService("CoreGui").devbuild.Enabled then
									Window.Enabled =  false
									Window:Fade(Window.Enabled)
									wait(0.26)
									game:GetService("CoreGui").devbuild.Enabled = false
								else
									Window.Enabled =  true
									Window:Fade(Window.Enabled)
									game:GetService("CoreGui").devbuild.Enabled = true
								end
							elseif Input.KeyCode and Input.KeyCode == Enum.KeyCode.L then
								Window:Unload()
							end
						end)
					end
								...
								if Key.EnumType then
									if Key.EnumType == Enum.KeyCode or Key.EnumType == Enum.UserInputType then
										if table.find(Keys.KeyCodes, Key.Name) or table.find(Keys.Inputs, Key.Name) then
											local letter = Key.Name:sub(-1) -- Extract the last character from Key.Name
											Content:Set({Key.EnumType == Enum.KeyCode and "KeyCode" or "UserInputType", letter})
											return true
										end
									end
								end
